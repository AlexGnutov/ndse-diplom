...
module.exports = (io) => {
    ...
    const getHistory = async function(msg) {
        ...
        console.log(socket.request.user); console.log(msg.receiver);
                ...
                if (foundChat) {
                    try {
                        ...
                        if (messages) {
                            ...
                            socket.emit('chatHistory', { text: strg });
                        }
                    } catch(e) {
                        console.log('Error calling getHistory: ', e.message);
                    }
                } else {
                    const strg = JSON.stringify({ error: `Chat doesn't exist` });
                    socket.emit('chatHistory', {text: strg});
                }
            } catch(e) {
